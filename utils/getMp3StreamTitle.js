...
import { URL } from "url";
export const getMp3StreamTitle = (streamingUrl) => {
  console.log("Fetching title for stream:", streamingUrl);
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.get(options, (res) => {
      console.log("Received response from stream URL:", streamingUrl);

      const icyMetaInt = parseInt(res.headers["icy-metaint"]);
      if (!icyMetaInt) {
        console.error("No icy-metaint found");
        return reject("No icy-metaint found");
      }
